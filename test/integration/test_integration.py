...
def create_test_data_sequence():
    ...
    # For Transolver, we need to match the H*W size with the model parameters
    train_features = np.random.randn(20, 100, 10)  # (batch, H*W, features)
    train_targets = np.random.randn(20, 100, 1)  # (batch, H*W, output)
    test_features = np.random.randn(10, 100, 10)  # (batch, H*W, features)
    test_targets = np.random.randn(10, 100, 1)  # (batch, H*W, output)
...
def test_mlp_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config_content = (
            config_content.replace("tabular_train_features.npz", os.path.join(data_dir, "tabular_train_features.npz"))
            .replace("tabular_train_targets.npz", os.path.join(data_dir, "tabular_train_targets.npz"))
            .replace("tabular_test_features.npz", os.path.join(data_dir, "tabular_test_features.npz"))
            .replace("tabular_test_targets.npz", os.path.join(data_dir, "tabular_test_targets.npz"))
        )

        # Create temporary config with updated output paths
        config_content = config_content.replace(
            "trained_mlp_model.pth", os.path.join(temp_dir, "trained_mlp_model.pth")
        ).replace("mlp_training_history.json", os.path.join(temp_dir, "mlp_training_history.json"))
...
def test_fno_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Replace paths with absolute paths
        config_content = (
            config_content.replace("grid_train_features.npz", os.path.join(data_dir, "grid_train_features.npz"))
            .replace("grid_train_targets.npz", os.path.join(data_dir, "grid_train_targets.npz"))
            .replace("grid_test_features.npz", os.path.join(data_dir, "grid_test_features.npz"))
            .replace("grid_test_targets.npz", os.path.join(data_dir, "grid_test_targets.npz"))
        )

        # Create temporary config with updated output paths
        config_content = config_content.replace(
            "trained_fno_model.pth", os.path.join(temp_dir, "trained_fno_model.pth")
        ).replace("fno_training_history.json", os.path.join(temp_dir, "fno_training_history.json"))
...
def test_transformer_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Replace paths with absolute paths
        config_content = (
            config_content.replace("sequence_train_features.npz", os.path.join(data_dir, "sequence_train_features.npz"))
            .replace("sequence_train_targets.npz", os.path.join(data_dir, "sequence_train_targets.npz"))
            .replace("sequence_test_features.npz", os.path.join(data_dir, "sequence_test_features.npz"))
            .replace("sequence_test_targets.npz", os.path.join(data_dir, "sequence_test_targets.npz"))
        )

        # Create temporary config with updated output paths
        config_content = config_content.replace(
            "trained_transformer_model.pth", os.path.join(temp_dir, "trained_transformer_model.pth")
        ).replace("transformer_training_history.json", os.path.join(temp_dir, "transformer_training_history.json"))
...
def test_meshgraphnet_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config_content = config_content.replace(
            ...
            "graph_train_edge_index.npz",
            os.path.join(data_dir, "graph_train_edge_index.npz")
        ).replace(
            "graph_train_edge_features.npz",
            os.path.join(data_dir, "graph_train_edge_features.npz")
        ).replace(
            "graph_test_edge_index.npz",
            os.path.join(data_dir, "graph_test_edge_index.npz")
        ).replace(
            "graph_test_edge_features.npz",
            os.path.join(data_dir, "graph_test_edge_features.npz")
        )
        
        # Create temporary config with updated output paths
        config_content = config_content.replace(
            "trained_meshgraphnet_model.pth",
            os.path.join(temp_dir, "trained_meshgraphnet_model.pth")
        ).replace(
            "meshgraphnet_training_history.json",
            os.path.join(temp_dir, "meshgraphnet_training_history.json")
        )
        ...
        try:
            train_main(temp_config)
            print("✅ MeshGraphNet integration test passed")
            return True
        except Exception as e:
...
def test_unet_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create temporary config with updated output paths
        config_content = config_content.replace(
            "trained_unet_model.pth", os.path.join(temp_dir, "trained_unet_model.pth")
        ).replace("unet_training_history.json", os.path.join(temp_dir, "unet_training_history.json"))
...
def test_transolver_integration():
    """Test Transolver integration with spatial data"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Update config file paths
        config_path = os.path.join(os.path.dirname(__file__), "transolver", "config_transolver.yaml")
        with open(config_path, "r") as f:
            config_content = f.read()
        ...
        # Create temporary config with updated output paths
        config_content = config_content.replace(
            "trained_transolver_model.pth", os.path.join(temp_dir, "trained_transolver_model.pth")
        ).replace("transolver_training_history.json", os.path.join(temp_dir, "transolver_training_history.json"))
        ...
        try:
            train_main(temp_config)
            print("✅ Transolver integration test passed")
            return True
        except Exception as e:
